...
'''
This helps to run SAM2 in a docker container as a gRPC server. Normal env runs do not work. 
Lines 21 to 25 are solution to run it on deployment servers. Local gRPC run tests work but fail on docker deployments
'''
...
class SamToolServicer(sam_tool_pb2_grpc.SamToolServicer):
    ...
    def remove_invalid_boxes(self, df, image_path, threshold, width, height):
        """
    ...
    def remove_large_boxes(self, df, image_path, size_threshold, width, height):
        """
    ...
    def remove_edge_boxes(self, df, image_path, edge_threshold, width, height):
        """
    ...
    def generate_bounding_boxes(self, image_dir, width, height):
        bbox_data = []

        
        # image = cv2.cvtColor(image_dir, cv2.COLOR_BGR2RGB)
        image = image_dir

        masks = self.mask_generator_6.generate(image)
    ...
    def apply_box_suppression(self, bbox_df, image, width, height):
        filtered_df = self.nms_from_csv(bbox_df, 0.2)
        print(f"After NMS: {filtered_df.shape[0]} boxes")
        
        filtered_df = self.remove_invalid_boxes(filtered_df, image, 0.8, width, height)
        ...
        filtered_df = self.remove_large_boxes(filtered_df, image, 0.25, width, height)
        print(f"After large box removal: {filtered_df.shape[0]} boxes")
        
        n_arr = self.remove_edge_boxes(filtered_df, image, 4, width, height)
    ...
    def main(self, request, context):
        ...
        # image_arr = base64.b64decode(data)
        image_arr = np.array(data, dtype=object)
        image_arr = [ast.literal_eval(img) for img in image_arr]
        image_np = np.array(image_arr,dtype=np.uint8).reshape(h,w,-1)
        # print(image_np, type(image_np), type(image_np[0]))
        # image_arr = Image.open(io.BytesIO(image_arr))
        bbox_df = self.generate_bounding_boxes(image_np, w, h)

        # print(image_arr, type(image_arr))

        final_df = self.apply_box_suppression(bbox_df, image_arr,w, h)
